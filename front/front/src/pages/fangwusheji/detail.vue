  ...
  <div>
    ...
    >
        ...
        >
          ...
          >
            ...
            <el-button
              ...
              v-if="btnAuth('fangwusheji', '问题')"
              @click="onAcross('zhuangxiuwenti', '否', '是', '', '')"
              type="warning"
              >问题</el-button
            >
            <!-- crossOptButtonStatusColumns  -->
            <el-button
              ...
              v-if="btnAuth('fangwusheji', '进度')"
              @click="onAcross('xiangmujindu', '否', '是', '', '')"
              type="warning"
              >进度</el-button
            >
            <el-button
      ...
      <el-carousel
        ...
        :style="{ width: '40%', margin: '0 5% 0 0', height: '100%' }"
        trigger="click"
      ...
      >
        ...
        <div :style="{ background: '#FFF' }" v-html="detail.shejifangan"></div>
      </div>
...
export default {
  ...
  methods: {
    init() {
      ...
      this.$http.get(this.tablename + "/detail/" + this.id, {}).then((res) => {
        if (res.data.code == 0) {
          ...
          this.detailBanner = this.detail.gongsifengmian
            ? this.detail.gongsifengmian.split(",")
            : [];
          this.$forceUpdate();
    ...
    async onAcross(
      ...
      localStorage.setItem("crossTable", `fangwusheji`);
    ...
    payClick() {
      localStorage.setItem("paytable", "fangwusheji");
    ...
    editClick() {
      this.$router.push(
        `/index/fangwushejiAdd?type=edit&&id=${this.detail.id}`
      );
    },
    // 删除
    delClick() {
      this.$confirm("是否删除此房屋设计？")
        .then((_) => {
          this.$http
            .post("fangwusheji/delete", [this.detail.id])
            .then((res) => {
              if (res.data.code == 0) {
                this.$message({
                  type: "success",
