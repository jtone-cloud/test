  ...
  <div>
    ...
    >
        ...
        >
          ...
          >
            ...
            <el-button
              ...
              v-if="btnAuth('xiangmujindu', '评价')"
              @click="onAcross('pingjiaxinxi', '', '', '', '')"
              type="warning"
              >评价</el-button
            >
          </div>
      ...
      >
        ...
        <div :style="{ background: '#FFF' }" v-html="detail.xiangmujindu"></div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async onAcross(
      ...
      localStorage.setItem("crossTable", `xiangmujindu`);
    ...
    editClick() {
      this.$router.push(
        `/index/xiangmujinduAdd?type=edit&&id=${this.detail.id}`
      );
    },
    // 删除
    delClick() {
      this.$confirm("是否删除此项目进度？")
        .then((_) => {
          this.$http
            .post("xiangmujindu/delete", [this.detail.id])
            .then((res) => {
              if (res.data.code == 0) {
                this.$message({
                  type: "success",
