...
export default {
  ...
  methods: {
    ...
    // 修改
    editClick() {
      this.$router.push(`/index/yezhuAdd?type=edit&&id=${this.detail.id}`);
    },
    // 删除
    delClick() {
      this.$confirm("是否删除此业主？")
        .then((_) => {
          this.$http.post("yezhu/delete", [this.detail.id]).then((res) => {
            if (res.data.code == 0) {
              this.$message({
                type: "success",
                message: "删除成功!",
